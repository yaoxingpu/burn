...
mod tests {
    ...
    #[test]
    fn should_diff_cat() {
        let tensor_1 = TestAutodiffTensor::from([[2.0, -1.0], [5.0, 2.0]]).require_grad();
        let tensor_2 = TestAutodiffTensor::from([[5.0, 4.0], [-1.0, 4.0]]).require_grad();
    ...
    fn should_diff_cat_more_than_1_dim() {
        let tensor_1 = TestAutodiffTensor::from([[2.0, -1.0], [5.0, 2.0]]).require_grad();
        let tensor_2 =
            TestAutodiffTensor::from([[5.0, 4.0], [-1.0, 4.0], [4.0, 1.0]]).require_grad();
